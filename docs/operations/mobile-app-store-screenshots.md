...
The Environments capture presents the three local fixture transports in the device selector as a
Tailscale HTTPS hostname, a Helsinki VPS hostname, and a Tailnet IPv4 address. This display-only
substitution keeps the screen remote-first while the harness retains reliable loopback connections
to its ephemeral servers.
