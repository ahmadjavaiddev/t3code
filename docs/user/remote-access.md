...
On mobile, open **Settings** → **Environments**, then tap a device. Its details screen keeps the
connection settings, pairing links, and authorized clients together. A management-capable mobile
session can:
...
Ordinary mobile pairings intentionally cannot manage other clients. If the environment details screen
asks for management permission, pair again with the owner link printed when the server starts (or a link
